...
enum class Colour {NONE, BLUE, RED};

std::ostream& operator<< (std::ostream &out, const Colour &c);
Colour operator++(Colour &in);
Colour operator++(Colour &in, int);
...
public:
    ...
    void printGraph();
    unsigned int V() const { return nodeCnt; }
    unsigned int E() const { return edgeCnt; }
    unsigned int D() const { return density; }
...
protected:
    ...
    void Add(Edge &edge);
    inline void setColour(unsigned int id, Colour c) { graph[id]->color = c; }
    inline Colour getColour(unsigned int id) const { return graph[id]->color; }
};
